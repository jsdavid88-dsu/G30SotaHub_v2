// SOTA 배정 — 테스트 영상/이미지 업로드 + 프레임별 노트 섹션.
// owner_type='sota_assignment' attachment 재사용. 영상 열고 "주석" 켜면
// VideoAnnotator(프레임 단위 ±1 스텝 + timecode 노트)가 그대로 동작.
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch(
        `/api/v1/attachments?owner_type=sota_assignment&owner_id=${assignmentId}`,
        { headers: authHeaders() },
      )
      if (res.ok) {
    ...
    <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid #f1f5f9' }}>
      ...
      {loaded && atts.length === 0 && (
        <p style={{ fontSize: 12, color: '#cbd5e1', marginBottom: 6 }}>
          아직 업로드된 테스트 자료가 없습니다.
        </p>
      )}

      {canUpload && (
        <AttachmentUploader
          ownerType="sota_assignment"
